...
# Человек может есть, работать, играть, ходить в магазин итд.
# У человека есть степень сытости, денег итд.
...
# Реализуем модель дом.
# В доме могут заселятся люди и коты.
# У дома есть параметры денег в доме, загрязненность, кол-во еды и кошачьей еды итд.
class House:
    def __init__(self):
        self.food = 50
        self.money = 0
...
# Реализуем модель кота.
# Кот может есть, спать и драть обои.
# У кота есть степень сытости.
# Если сытость = 0 единиц, кот умирает.
# коту надо прожить вместе с человеком в доме 365 дней.
class Cat:
...
# Создадим трех людей, живущих в одном доме
...
# Иногда люди будут подбирать с улицы котов и приносить их в дом, чтобы заботиться о котах
